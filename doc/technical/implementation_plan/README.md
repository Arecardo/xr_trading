...
| BT-003a | DONE（2026-08-16） | 详见 `04_backtest_matching_strategy_risk.md` §3.1；`unrestricted`/`restricted` 精度模式对比脚本 + 新增 `compute_tracking_error` 逐日跟踪误差计算；**关键发现**：默认参数下 `restricted` 模式因 BT-003b 的订单收缩与整股取整叠加，绝大多数调仓被跳过，NVDA/QQQ 平均跟踪误差达 19%~20%（远高于预期），已产出四个可选应对方向供用户裁决，见需求文档 §5.1.1「验证结果」；用的是种子固定的示例性行情数据，非真实长桥历史行情（本环境无真实凭据） |
