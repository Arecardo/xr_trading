...
2. [组合估值与行情集成](./02_portfolio_and_market_integration.md)（BE-003、BE-004a、BE-004）
...
| M2 组合估值与行情集成 | 估值快照生成 + 行情服务契约打通（BE-003、BE-004a、BE-004） | 净值可逐项对账；三个标的精度规则可通过批量 API 一次查到；两服务不共表 |
...
| W4 | BE-003 组合估值快照、BE-004 行情集成客户端、BT-004 策略接口实现、BT-005 风控接口实现 | Strategy/RiskPolicy 接口签名、BE-004a 已上线 |
