...
| BT-003a | DONE（2026-08-16，同日两轮补充分析） | 详见 `04_backtest_matching_strategy_risk.md` §3.1；`unrestricted`/`restricted` 精度模式对比脚本 + 新增 `compute_tracking_error` 逐日跟踪误差计算；**关键发现**：默认参数下 `restricted` 模式因 BT-003b 的订单收缩与整股取整叠加，绝大多数调仓被跳过，NVDA/QQQ 平均跟踪误差达 19%~20%（远高于预期）；**根因最终定位为 NVDA/QQQ 之间的价差（QQQ 贵 NVDA 4 倍），不是任一标的的绝对价格**——只需让组合内标的价格档位接近 + 风险预算温和上调到 1%（仍在需求文档原定 0.5%-1% 区间内），两个标的的跟踪误差即可收敛到接近 unrestricted 基线，是目前最优的组合方案，见需求文档 §5.1.1；用的是种子固定的示例性行情数据，非真实长桥历史行情（本环境无真实凭据） |
