...
| **BT-003a 补充：敏感性扫描（2026-08-16）** | DONE | BT-003a | 脚本新增三组扫描（同一份数据）：初始资金（$2,500/$10,000/$25,000）、`max_order_risk_pct_of_nav`（0.5%/1%/1.6%/2%）、标的单价水平（原价/降至 1/4）。量化结论见需求文档 §5.1.1「补充实测」：初始资金提到 $25,000 或风险预算提到 1.6% 均可把跟踪误差收敛到 unrestricted 基线附近；**单独调到 1% 风险预算不够**（只解决 NVDA，QQQ 单价是 NVDA 的 4 倍，仍不足 1 股）；1.6% 已超出需求文档原定「0.5%-1%」区间，是风险容忍度取舍，不是免费的参数调整；降低标的单价与提高资金效果等价（决定因素是「收缩后名义金额/单价」比值），但选具体替代标的超出本任务职责，不做投资建议 | 3 组扫描共 10 次额外回测运行，均可重复；`ruff`/`mypy`/`pytest`/`security-check` 全绿 |
