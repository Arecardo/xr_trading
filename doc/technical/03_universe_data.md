...
2.1. **例外（2026-08-05，RM0 DEC-006）**：`FX` 类型 Instrument（汇率参考数据，如 `fx:coingecko:usdt-usd`）不参与本流程——它不出现在任何组合的成员/持仓/基准中，采集范围由"资产宇宙中出现的非基础货币计价币种"驱动，只要存在这类持仓（如 `BTC-USDT`）就必须持续采集，与组合成员变化无关。
