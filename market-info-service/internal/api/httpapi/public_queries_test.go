...
func TestRegisterPublicQueryRoutes(t *testing.T) {
	...
	response := httptest.NewRecorder()
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodPost, paths[0], nil))
...
func TestRegisterPublicQueryRoutesRejectsMissingDependencies(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "instrument options", mux: http.NewServeMux(), routes: PublicQueryRoutes{LatestQuotes: valid.LatestQuotes, Bars: valid.Bars}, wantError: "instrument options"},
		{name: "latest quotes", mux: http.NewServeMux(), routes: PublicQueryRoutes{InstrumentOptions: valid.InstrumentOptions, Bars: valid.Bars}, wantError: "latest quote"},
		{name: "bars", mux: http.NewServeMux(), routes: PublicQueryRoutes{InstrumentOptions: valid.InstrumentOptions, LatestQuotes: valid.LatestQuotes}, wantError: "bars"},
	}
